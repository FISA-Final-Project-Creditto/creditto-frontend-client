...
export default function RecurringHistory({ record, onClose }) {
  ...
  const tRecurring = useTranslations("send.regular.history.recurring");
  // ,(쉼표)를 3자리로 끊어서 설정
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-bold text-center mb-4">{tRecurring("detail_title")}</h2>

      <div className="space-y-4 pt-2">
        <div className="border border-[#E5E6EB] bg-[#F7F8FA] rounded-xl p-4 text-center">
          <p className="text-sm text-[#C9CDD4] mb-1">{tRecurring("transaction_amount_label")}</p>
        ...
        <div className="space-y-3">
          <div className="flex justify-between py-3 ">
            <span className="text-[#86909C]">{tRecurring("balance_label")}</span>
            <span className="font-semibold">{record.accountNo}</span>
          </div>

          <div className="flex justify-between py-3 ">
            <span className="text-[#86909C]">{tRecurring("total_fee_label")}</span>
          ...
          <div className="flex justify-between py-3 ">
            <span className="text-[#86909C]">{tRecurring("recipient_bank_name_label")}</span>
            <span className="font-semibold">{record.recipientBankName}</span>
          </div>

          <div className="flex justify-between py-3 ">
            <span className="text-[#86909C]">{tRecurring("recipient_account_number_label")}</span>
            <span className="font-semibold">{record.recipientAccountNo}</span>
          </div>

          <div className="flex justify-between py-3">
            <span className="text-[#86909C]">{tRecurring("remittance_status_label")}</span>
